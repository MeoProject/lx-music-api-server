        ...
        res.append(
            {
                ...
                "singer_list": [
                    {"name": i["name"], "id": i["id"]} for i in songinfo["Singers"]
                ],
                ...
                "language": (
                    songinfo["trans_param"].get("language")
                    if songinfo["trans_param"]
                    else ""
                ),
...
async def getSongSearchResult(query, page=1, size=20):
    ...
    req = await request.AsyncRequest(
        utils.encodeURI(
            f"https://songsearch.kugou.com/song_search_v2?"
            + buildRequestParams(
                {
                    "keyword": query,
                    "page": page,
        ...
        res.append(
            {
                ...
                "mvid": songinfo["MvHash"],
                "subresult": (
                    [] if (songinfo["Grp"] == []) else formatSubResult(songinfo["Grp"])
                ),
            }
        )
    return {
