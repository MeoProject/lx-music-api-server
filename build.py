...
def get_latest_tag():
    ...
    return (
        t[-1]
        if (t[-1] != toml.load("./pyproject.toml")["tool"]["poetry"]["version"])
        else t[-2]
    )
...
def get_changelog():
    ...
    for msg in res:
        if re.match(
            "[a-f0-9]*.(feat|fix|docs|style|refactor|perf|test|build|ci|chore|revert|notice).*?\(?.*?\)?\:",
            msg[1:-1],
        ):
            msg = msg[1:-1]
...
def build_test(fileName):
    ...
    if popen.returncode != 0:
        ...
        print(
            "FilePath: " + os.getcwd() + os.sep + os.listdir(os.getcwd() + "/dist")[0]
        )
...
def main():
    ...
    for o in options:
        if o == "-f" or o == "--fileName":
            ...
            print("Options:")
            print(
                "  -f, --filename <fileName>  Specify the fileName of the executable."
            )
